...
module.exports = {
  ...
  // 在 webpack 4.x 中，有一个很大的特性，是 约定大于配置  约定，默认的打包入口路径是 src -> index.js
  plugins: [
    htmlPlugin
  ],
  module: { // 所有第三方 模块的配置规则
    rules: [ // 第三方匹配规则
      ...
      // 可以在 css-loader 之后，通过 ？追加参数，其中，有个固定的参数，叫做 modules,表示为普通的 css 样式表，启用模块化
      // 自己规定第三方都是以 .css 结尾, 这样我们不要以普通的 .css; 自己的样式表, 都要以 .scss 或 .less 结尾
